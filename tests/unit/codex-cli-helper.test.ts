...
describe('CodexCliHelper', () => {
  ...
  describe('getRateLimit', () => {
    ...
    it('should count entries within the 5-hour window', () => {
      ...
      // Write entries within the window
      const now = new Date()
      const entries = [
    ...
    it('should exclude entries outside the 5-hour window', () => {
      ...
      const now = new Date()
      const entries = [
        { timestamp: new Date(now.getTime() - 1000 * 60 * 60).toISOString() }, // 1 hour ago (in window)
        { timestamp: new Date(now.getTime() - 1000 * 60 * 60 * 6).toISOString() }, // 6 hours ago (out)
        { timestamp: new Date(now.getTime() - 1000 * 60 * 60 * 24).toISOString() }, // 24 hours ago (out)
      ]
      fs.writeFileSync(
        TEST_USAGE_LOG,
        entries.map((e) => JSON.stringify(e)).join('\n') + '\n'
    ...
    it('should report near limit when usage exceeds 80%', () => {
      ...
      // Write 240+ entries (80% of 300) within the window
      const now = new Date()
